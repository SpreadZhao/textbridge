...
App 单屏包含电脑地址、发现端口、扫描电脑、访问令牌、多行文本框、发送按钮和发送历史。扫描只发现电脑地址，不传输或填充访问令牌。发现端口需要和 `services.textbridge.server.discovery.port` 一致。地址、发现端口、令牌和最近 50 条成功发送历史保存在 DataStore；只有服务端返回 `status=ok` 时才清空正文并写入历史，失败会保留正文且不记录历史。
